...
The user will deploy to a shared host via FTP / File Manager. A few notes:

- Most shared hosts serve `.webp` correctly with the right MIME type out of the box (verify with the `.htaccess` MIME block if not).
...
- For multi-page sites, `/about.html`, `/contact.html` etc. work as `yourdomain.com/about` and `yourdomain.com/contact` if the host has "Pretty URLs" enabled (or the rewrite rule in `templates/htaccess.template` is uncommented). Otherwise, use full `.html` extension in links.
...
If both pass, the upload to the live host will work too.
